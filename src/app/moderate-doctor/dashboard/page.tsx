...
export default function ModerateDoctorDashboard() {
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <div className="flex items-center gap-2 text-teal-600 mb-1">
                            ...
                            <span className="text-xs font-bold uppercase tracking-widest">Clinical Review Dashboard</span>
                        </div>
                        <h1 className="text-4xl font-black tracking-tight text-slate-900">Welcome, Moderator</h1>
                        <p className="text-slate-500 font-medium">Monitoring {cases.length} active clinical loops for quality assurance.</p>
                    </div>
                </div>

                {/* Stats */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <StatCard title="Total Audits" value={cases.length.toString()} icon={FileSearch} color="blue" />
                    <StatCard title="Pending Review" value={cases.filter(c => c.status !== 'Under Review').length.toString()} icon={Clock} color="amber" />
                    <StatCard title="Quality Score" value="98.4%" icon={BarChart3} color="emerald" />
                </div>
                ...
                <Card className="border-slate-200 shadow-xl shadow-slate-200/50 bg-white">
                    <CardHeader className="pb-4 border-b border-slate-50">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <CardTitle className="text-xl font-bold">Active Consultations</CardTitle>
                                <CardDescription>Review doctor assessments and patient interactions.</CardDescription>
                            </div>
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 text-slate-500 text-xs font-bold uppercase tracking-wider">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4">Clinical Team</th>
                                    ...
                                    {filteredCases.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-medium">
                                                No cases currently requiring moderation.
                                            </td>
                                        </tr>
                                    ) : filteredCases.map(c => {
                                            ...
                                            <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                                                <td className="px-6 py-6">
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-10 w-10 bg-teal-50 rounded-xl flex items-center justify-center text-teal-600 font-bold border border-teal-100">
                                                            {c.id.slice(-2)}
                                                        </div>
                                                        <div>
                                                            <div className="font-bold text-slate-900">{c.id}</div>
                                                            <div className="text-xs text-slate-500 font-medium">{c.type} &middot; {c.urgency}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-6">
                                                    <div className="space-y-1">
                                                        <div className="flex items-center gap-2 text-sm">
                                                            ...
                                                            <span className="font-semibold text-slate-700">{patient?.firstName} {patient?.lastName}</span>
                                                        </div>
                                                ...
                                                <td className="px-6 py-6">
                                                    <Badge variant="outline" className={`font-bold transition-all ${
                                                        c.status === 'Under Review' ? 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse' :
                                                ...
                                                <td className="px-6 py-6 text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <Button 
                                                            variant="ghost" 
                                                            className="h-9 px-3 text-slate-500 hover:text-blue-600 hover:bg-blue-50 font-bold text-xs"
                                                        ...
                                                        <Button 
                                                            variant="ghost" 
                                                            className="h-9 px-3 text-slate-500 hover:text-red-600 hover:bg-red-50 font-bold text-xs"
                                                        ...
                                                        <Button 
                                                            className="h-9 px-4 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs rounded-lg shadow-lg shadow-teal-600/20"
...
function StatCard({ title, value, icon: Icon, color }: any) {
    ...
    return (
        <Card className="border-slate-200 shadow-lg shadow-slate-200/30 overflow-hidden transform hover:-translate-y-1 transition-all duration-300">
